...
contract SiloGovernorDeploy is CommonDeploy {
    ...
    function run()
        public
        returns (ISiloGovernor siloGovernor, ISiloTimelockController timelock, IVeSilo votingEscrow, IVeBoost veBoost)
    {
        ...
        siloGovernor = ISiloGovernor(
            address(new SiloGovernor(TimelockController(payable(address(timelock))), votingEscrow))
        );
