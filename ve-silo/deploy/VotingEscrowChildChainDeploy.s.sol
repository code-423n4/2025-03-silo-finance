...
contract VotingEscrowChildChainDeploy is CommonDeploy {
    ...
    function run() public returns (IVotingEscrowChildChain votingEscrow) {
        ...
        bytes memory data = abi.encodeWithSelector(VotingEscrowChildChain.initialize.selector, owner);

        vm.startBroadcast(deployerPrivateKey);

        address implementation = address(new VotingEscrowChildChain(chainlinkCcipRouter, _sourceChainSelector()));
