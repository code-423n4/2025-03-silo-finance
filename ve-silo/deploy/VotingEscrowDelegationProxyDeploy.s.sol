...
import {IVotingEscrowDelegationProxy} from "ve-silo/contracts/voting-escrow/interfaces/IVotingEscrowDelegationProxy.sol";
...
contract VotingEscrowDelegationProxyDeploy is CommonDeploy {
    function run() public returns (IVotingEscrowDelegationProxy proxy) {
        ...
        proxy = IVotingEscrowDelegationProxy(
            address(new VotingEscrowDelegationProxy(IERC20(nullVotingEscrow), IVeDelegation(veBoost)))
        );
