...
contract VeSiloDelegatorViaCCIPDeploy is CommonDeploy {
    function run() public returns (IVeSiloDelegatorViaCCIP delegator) {
        ...
        bytes memory data = abi.encodeWithSelector(VeSiloDelegatorViaCCIP.initialize.selector, timelock);

        vm.startBroadcast(deployerPrivateKey);

        address implementation = address(
            new VeSiloDelegatorViaCCIP(IVeSilo(veSilo), IVotingEscrowCCIPRemapper(remapper), chainlinkCCIPRouter, link)
        );
