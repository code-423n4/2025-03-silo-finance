...
import {ISiloFactoryWithFeeDetails as ISiloFactory} from "ve-silo/contracts/silo-tokens-minter/interfaces/ISiloFactoryWithFeeDetails.sol";
...
contract MainnetTest is IntegrationTest {
    ...
    function _updateBalanceInGauge(
        ...
        vm.mockCall(_shareToken, abi.encodeCall(IShareToken.balanceOf, _user), abi.encode(_balance));
    ...
    function _activeteBlancerTokenAdmin() internal {
        stdstore.target(getAddress(SILO_TOKEN)).sig(IExtendedOwnable.owner.selector).checked_write(
            address(_balancerTokenAdmin)
        );
    ...
    function _configureFakeSmartWalletChecker() internal {
        ...
        vm.mockCall(_smartValletChecker, abi.encodeCall(ISmartWalletChecker.check, _bob), abi.encode(true));

        vm.mockCall(_smartValletChecker, abi.encodeCall(ISmartWalletChecker.check, _alice), abi.encode(true));

        vm.mockCall(_smartValletChecker, abi.encodeCall(ISmartWalletChecker.check, _john), abi.encode(true));

        vm.mockCall(_smartValletChecker, abi.encodeCall(ISmartWalletChecker.check, _daoVoter), abi.encode(true));
    }
    ...
    function _mockSiloFeesDetails() internal {
        ...
        vm.mockCall(
            ...
            abi.encode(_daoFeeReceiver, _deployerFeeReceiver)
        );
    ...
    function _executeProposal(Proposal _proposal) internal {
        ...
        _siloGovernor.queue(targets, values, calldatas, descriptionHash);

        vm.warp(block.timestamp + 3_600);

        _siloGovernor.execute(targets, values, calldatas, descriptionHash);
    }
    ...
    function _mockSiloCore() internal {
        ...
        vm.mockCall(_shareToken, abi.encodeWithSelector(IShareToken.hookReceiver.selector), abi.encode(_hookReceiver));

        vm.mockCall(_shareToken, abi.encodeWithSelector(IShareToken.silo.selector), abi.encode(_silo));

        vm.mockCall(_silo, abi.encodeWithSelector(ISilo.factory.selector), abi.encode(_siloFactory));
    }
