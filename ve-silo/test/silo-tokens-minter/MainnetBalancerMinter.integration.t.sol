...
import {MainnetBalancerMinterDeploy, IBalancerTokenAdmin, IBalancerMinter} from "ve-silo/deploy/MainnetBalancerMinterDeploy.s.sol";
...
contract MainnetBalancerMinterTest is IntegrationTest {
    ...
    function testOnlyOwnerCanSetFees() public {
        _feesTest.onlyOwnerCanSetFees(IFeesManager(address(_minter)), _DAO_FEE, _DEPLOYER_FEE, _deployer);
    }

    function testMaxFees() public {
        _feesTest.onlyOwnerCanSetFees(IFeesManager(address(_minter)), _DAO_FEE, _DEPLOYER_FEE + 1, _deployer);
    }
    ...
    function _hashData(
        ...
        structHash = keccak256(abi.encode(_SET_MINTER_APPROVAL_TYPEHASH, userMinter, approval, userNonce, deadline));
    }
