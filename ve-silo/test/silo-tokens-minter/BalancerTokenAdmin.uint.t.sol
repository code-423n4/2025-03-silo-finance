...
contract BalancerTokenAdminTest is IntegrationTest {
    uint256 internal constant _INITIAL_SUPPLY = 1000;
    ...
    function testBalanceTokenAdminMint() public {
        ...
        vm.mockCall(_token, payload, abi.encode(true));
    ...
    function _mockToken() internal {
        vm.mockCall(_token, abi.encodeWithSelector(Ownable.owner.selector), abi.encode(address(_tokenAdmin)));

        vm.mockCall(_token, abi.encodeWithSelector(IERC20.totalSupply.selector), abi.encode(_INITIAL_SUPPLY));
    }
}
