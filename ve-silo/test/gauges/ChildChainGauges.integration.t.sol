...
contract ChildChainGaugesTest is IntegrationTest {
    ...
    function testOnlyOwnerCanSetFees() public {
        _feesTest.onlyOwnerCanSetFees(IFeesManager(address(_factory)), _DAO_FEE, _DEPLOYER_FEE, _deployer);
    }

    /// @notice Should revert if fees are invalid
    function testMaxFees() public {
        _feesTest.onlyOwnerCanSetFees(IFeesManager(address(_factory)), _DAO_FEE, _DEPLOYER_FEE + 1, _deployer);
    }
    ...
    function testCreateChildChainGaugeAndVerifyGetters() public {
        ...
        assertEq(gauge.authorizer_adaptor(), getAddress(AddrKey.L2_MULTISIG), "Deployed with wrong timelockController");
    }
    ...
    function testTransferTokensToTheMinter() public {
        ...
        assertEq(_siloToken.balanceOf(address(_l2BalancerPseudoMinter)), 0, "Before. An invalid balance of the minter");
