...
import {IGaugeAdder, ILiquidityGaugeFactory} from "ve-silo/contracts/gauges/gauge-adder/GaugeAdder.sol";
...
contract GaugeAdderTest is IntegrationTest {
    ...
    function _mockCalls() internal {
        vm.mockCall(
            _controller,
            abi.encodeWithSelector(IGaugeController.add_gauge.selector, _gauge, _ETHEREUM_GAUGE_CONTROLLER_TYPE),
            abi.encode(true)
        );
        ...
        vm.mockCall(
            _factory,
            abi.encodeWithSelector(ILiquidityGaugeFactory.isGaugeFromFactory.selector, address(0)),
            abi.encode(false)
        );
    }
}
