...
import {IMainnetBalancerMinter, ILMGetters, IBalancerMinter} from "ve-silo/contracts/silo-tokens-minter/interfaces/IMainnetBalancerMinter.sol";

import {IStakelessGaugeCheckpointerAdaptor} from "ve-silo/contracts/gauges/interfaces/IStakelessGaugeCheckpointerAdaptor.sol";
...
contract CCIPGaugeCheckpointer is IntegrationTest {
    string internal constant _GAUGE_TYPE = "Ethereum";
    ...
    function _mockCallsBeforeGaugeCreation() internal {
        ...
        vm.mockCall(_tokenAdmin, abi.encodeWithSelector(IBalancerTokenAdmin.RATE_DENOMINATOR.selector), abi.encode(1));
        ...
        vm.mockCall(_tokenAdmin, abi.encodeWithSelector(IBalancerTokenAdmin.rate.selector), abi.encode(1e3));
    }
    ...
    function _mockCallsAfterGaugeCreated(address _gaugeToMock) internal {
        ...
        vm.mockCall(_minter, abi.encodeWithSelector(IBalancerMinter.mint.selector, _gaugeToMock), abi.encode(true));

        vm.mockCall(
            _gaugeAdder,
            abi.encodeWithSelector(IGaugeAdder.isValidGaugeType.selector, _GAUGE_TYPE),
            abi.encode(true)
        );

        vm.mockCall(
            _gaugeAdder,
            abi.encodeWithSelector(IGaugeAdder.getFactoryForGaugeType.selector, _GAUGE_TYPE),
            abi.encode(_gaugeFactory)
        );

        vm.mockCall(
            _gaugeController,
            abi.encodeWithSelector(IGaugeController.gauge_exists.selector, _gaugeToMock),
            abi.encode(true)
        );
    }

    function _updateChainlinkPriceFeed() internal {
        IChainlinkPriceFeedLike.TokenPriceUpdate[] memory tokenPrices = new IChainlinkPriceFeedLike.TokenPriceUpdate[](
            3
        );
