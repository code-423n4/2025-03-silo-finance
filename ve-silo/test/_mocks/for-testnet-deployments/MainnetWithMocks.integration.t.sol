...
contract MainnetWithMocksIntegrationTest is MainnetTest {
    uint256 public constant ARBITRUM_FORKING_BLOCK = 169076190;

    function setUp() public override {
        // disabling `ve-silo/deploy/MainnetDeploy.s.sol` deployment
        _executeMainnetDeploy = false;

        vm.createSelectFork(getChainRpcUrl(ARBITRUM_ONE_ALIAS), ARBITRUM_FORKING_BLOCK);
    ...
    function testTransferVotingPowerCCIP() public {
        ...
        vm.prank(_deployer);
        veSiloDelegator.sendUserBalance{value: fee}(_deployer, dstChainSelector, ICCIPMessageSender.PayFeesIn.Native);
    }
    ...
    function _executeChangeGaugeWeightProposal(address _gauge) internal {
        ...
        gaugeWeightProposal.setGauge(_gauge).setWeight(1e18).setProposerPK(_daoVoterPK).run();
    ...
    function _checkpointerAndFees() internal returns (ICCIPGaugeCheckpointer ccipCheckpointer, uint256 ethFees) {
        ...
        ethFees = ccipCheckpointer.getTotalBridgeCost(0, Constants._GAUGE_TYPE_CHILD, ICCIPGauge.PayFeesIn.Native);
    }
}
