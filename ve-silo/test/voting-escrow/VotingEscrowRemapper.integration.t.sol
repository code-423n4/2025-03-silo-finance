...
contract VotingEscrowRemapperTest is IntegrationTest {
    ...
    function setUp() public {
        vm.createSelectFork(getChainRpcUrl(SEPOLIA_ALIAS), _FORKING_BLOCK_NUMBER);
        ...
        vm.mockCall(_smartValletChecker, abi.encodeCall(ISmartWalletChecker.check, _localUser), abi.encode(true));
    ...
    function _clearNetworkRemapping(ICCIPMessageSender.PayFeesIn _payFeesIn, uint256 _fee) internal {
        ...
        emit SentUserBalance(_DS_CHAIN_SELECTOR, _remoteUser, _remoteUser, emptyUserPoint, totalSupplyPoint);

        vm.expectEmit(false, false, false, true);
        emit SentUserBalance(_DS_CHAIN_SELECTOR, _localUser, _localUser, userPoint, totalSupplyPoint);

        vm.prank(_localUser);

        if (_payFeesIn == ICCIPMessageSender.PayFeesIn.Native) {
            remapper.clearNetworkRemapping{value: _fee}(_localUser, _DS_CHAIN_SELECTOR, _payFeesIn);
        } else {
            remapper.clearNetworkRemapping(_localUser, _DS_CHAIN_SELECTOR, _payFeesIn);
        }
    }

    function _setNetworkRemapping(ICCIPMessageSender.PayFeesIn _payFeesIn, uint256 _fee) internal {
        uint userEpoch = votingEscrow.user_point_epoch(_localUser);
        IVeSilo.Point memory userPoint = votingEscrow.user_point_history(_localUser, userEpoch);
        ...
        vm.expectEmit(false, false, false, true);
        emit SentUserBalance(_DS_CHAIN_SELECTOR, _localUser, _remoteUser, userPoint, totalSupplyPoint);

        vm.prank(_localUser);

        if (_payFeesIn == ICCIPMessageSender.PayFeesIn.Native) {
            remapper.setNetworkRemapping{value: _fee}(_localUser, _remoteUser, _DS_CHAIN_SELECTOR, _payFeesIn);
        } else {
            remapper.setNetworkRemapping(_localUser, _remoteUser, _DS_CHAIN_SELECTOR, _payFeesIn);
        }
    }
