...
contract VeBoostV2Test is IntegrationTest {
    string internal constant _NAME = "Vote-Escrowed Boost";
    string internal constant _SYMBOL = "veBoost";
    string internal constant _VERSION = "v2.0.0";

    bytes32 internal constant _EIP712_TYPEHASH =
    ...
    function testVeBoostSignatureForOtherToken() public {
        ...
        bytes32 invalidDomainSeparator = keccak256(
            abi.encode(
                _EIP712_TYPEHASH,
        ...
        bytes32 invalidDigest = keccak256(
            abi.encode(
                invalidDomainSeparator,
                keccak256(abi.encode(PERMIT_TYPEHASH, _holder.addr, _spender, value, nonce, deadline))
            )
        );
        ...
        return
            keccak256(
                abi.encodePacked(
                    "\x19\x01",
                    _domainSeparatorV4(),
                    keccak256(abi.encode(PERMIT_TYPEHASH, _owner, _spenderAddr, _value, _nonce, _deadline))
                )
            );
    }
