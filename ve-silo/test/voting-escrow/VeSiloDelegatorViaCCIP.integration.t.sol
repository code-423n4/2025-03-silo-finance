...
contract VeSiloDelegatorViaCCIPTest is IntegrationTest {
    ...
    function testSetExtraArgs() public {
        ...
        assertEq(keccak256(delegator.extraArgs()), keccak256(anyExtraArgs), "Args did not match after the config");
    ...
    function testUnsupportedChain() public {
        vm.expectRevert(
            abi.encodeWithSelector(IVeSiloDelegatorViaCCIP.ChainIsNotSupported.selector, _DS_CHAIN_SELECTOR)
        );

        veSiloDelegator.sendUserBalance(_localUser, _DS_CHAIN_SELECTOR, ICCIPMessageSender.PayFeesIn.Native);

        vm.expectRevert(
            abi.encodeWithSelector(IVeSiloDelegatorViaCCIP.ChainIsNotSupported.selector, _DS_CHAIN_SELECTOR)
        );

        veSiloDelegator.sendTotalSupply(_DS_CHAIN_SELECTOR, ICCIPMessageSender.PayFeesIn.Native);
    }
    ...
    function testSendTotalSupplyNativeFee() public {
        ...
        uint256 fee = veSiloDelegator.estimateSendTotalSupply(_DS_CHAIN_SELECTOR, ICCIPMessageSender.PayFeesIn.Native);
    ...
    function testSendTotalSupplyLINKFee() public {
        ...
        uint256 fee = veSiloDelegator.estimateSendTotalSupply(_DS_CHAIN_SELECTOR, ICCIPMessageSender.PayFeesIn.LINK);
    ...
    function _sendUserBalance(ICCIPMessageSender.PayFeesIn _payFeesIn, uint256 _fee) internal {
        ...
        if (_payFeesIn == ICCIPMessageSender.PayFeesIn.Native) {
            veSiloDelegator.sendUserBalance{value: _fee}(_localUser, _DS_CHAIN_SELECTOR, _payFeesIn);
        } else {
            veSiloDelegator.sendUserBalance(_localUser, _DS_CHAIN_SELECTOR, _payFeesIn);
        }
    }
