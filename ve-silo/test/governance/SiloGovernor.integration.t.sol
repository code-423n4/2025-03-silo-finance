...
contract SiloGovernorTest is IntegrationTest {
    ...
    function _configureFakeSmartWalletChecker() internal {
        ...
        assertEq(_votingEscrow.smart_wallet_checker(), _fakeChecker, "Failed to configure a fake smart wallet checker");

        vm.mockCall(_fakeChecker, abi.encodeCall(ISmartWalletChecker.check, _voter), abi.encode(true));
    }
    ...
    function _executeProposals(address _checker) internal {
        ...
        calldatas[0] = abi.encodeWithSelector(_votingEscrow.commit_smart_wallet_checker.selector, _checker);
        ...
        uint256 proposalId = _siloGovernor.propose(targets, values, calldatas, description);
        ...
        _siloGovernor.queue(targets, values, calldatas, keccak256(bytes(description)));

        vm.warp(block.timestamp + 3_600);

        _siloGovernor.execute(targets, values, calldatas, keccak256(bytes(description)));
    }
}
