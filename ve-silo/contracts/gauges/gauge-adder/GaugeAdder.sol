...
contract GaugeAdder is IGaugeAdder, Ownable2Step, ReentrancyGuard {
    ...
    /// @inheritdoc IGaugeAdder
    function getFactoryForGaugeType(
        string memory gaugeType
    ) external view override withValidGaugeType(gaugeType) returns (ILiquidityGaugeFactory) {
        return _gaugeTypeFactory[gaugeType];
    }

    /// @inheritdoc IGaugeAdder
    function isGaugeFromValidFactory(
        address gauge,
        string memory gaugeType
    ...
    function addGauge(
        address gauge,
        string memory gaugeType
    ) external override onlyOwner withValidGaugeType(gaugeType) {
        _addGauge(gauge, gaugeType);
    }

    /// @inheritdoc IGaugeAdder
    function setGaugeFactory(
        ILiquidityGaugeFactory factory,
        string memory gaugeType
    ) external override onlyOwner withValidGaugeType(gaugeType) {
