...
contract CCIPGaugeCheckpointer is ICCIPGaugeCheckpointer, ReentrancyGuard, Ownable2Step {
    ...
    error NotEnougtFundsToPayFees(uint256 fees, uint256 balance, ICCIPGauge.PayFeesIn payFeesIn);
    ...
    function checkpointGaugesOfTypeAboveRelativeWeight(
        ...
        uint256 minRelativeWeight,
        ICCIPGauge.PayFeesIn payFeesIn
    ) external payable override nonReentrant withValidGaugeType(gaugeType) {
        uint256 currentPeriod = _roundDownBlockTimestamp();
    ...
    function addGaugesWithVerifiedType(
        string calldata gaugeType,
        ICCIPGauge[] calldata gauges
    ) external override withValidGaugeType(gaugeType) onlyOwner {
    ...
    function addGauges(
        string calldata gaugeType,
        ICCIPGauge[] calldata gauges
    ) external override withValidGaugeType(gaugeType) {
    ...
    function hasGauge(
        string calldata gaugeType,
        ICCIPGauge gauge
    ) external view override withValidGaugeType(gaugeType) returns (bool) {
    ...
    function getTotalGauges(
        string calldata gaugeType
    ) external view override withValidGaugeType(gaugeType) returns (uint256) {
    ...
    function getGaugeAtIndex(
        string calldata gaugeType,
        uint256 index
    ) external view override withValidGaugeType(gaugeType) returns (ICCIPGauge) {
    ...
    function _addGauges(string memory gaugeType, ICCIPGauge[] calldata gauges, bool isGaugeTypeVerified) internal {
        EnumerableSet.AddressSet storage gaugesForType = _gauges[gaugeType];
