...
abstract contract CCIPMessageSender is ICCIPMessageSender {
    ...
    function _sendMesssageViaCCIP(
        ...
        if (_payFeesIn == PayFeesIn.LINK) {
            ...
            messageId = IRouterClient(ROUTER).ccipSend(_dstChainSelector, ccipMessage);
        } else {
            messageId = IRouterClient(ROUTER).ccipSend{value: fee}(_dstChainSelector, ccipMessage);
        }
    ...
    function _calculateFee(
        ...
        fee = IRouterClient(ROUTER).getFee(_dstChainSelector, _ccipMessage);
    }
}
