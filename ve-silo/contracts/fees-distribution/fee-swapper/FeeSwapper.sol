...
contract FeeSwapper is FeeSwapperConfig {
    ...
    IERC20 public immutable SILO_TOKEN;
    IFeeDistributor public immutable FEE_DISTRIBUTOR;
    Vault public immutable BALANCER_VAULT;
    ...
    /// @inheritdoc IFeeSwapper
    function swapFees(address[] calldata _assets, bytes[] memory _data) external virtual onlyManager {
        _swapFees(_assets, _data);
    }
    ...
    function _swapViaBalancer(uint256 _siloExpectedAmount) internal virtual {
        ...
        Vault.FundManagement memory funds = Vault.FundManagement(address(this), false, payable(address(this)), false);
