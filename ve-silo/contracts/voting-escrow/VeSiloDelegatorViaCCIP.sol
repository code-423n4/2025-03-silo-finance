...
contract VeSiloDelegatorViaCCIP is CCIPMessageSender, Ownable2Step, Initializable, IVeSiloDelegatorViaCCIP {
    ...
    function sendUserBalance(address _localUser, uint64 _dstChainSelector, PayFeesIn _payFeesIn) external payable {
        ...
        _sendMesssageViaCCIP(_dstChainSelector, childChainReceiver, data, _payFeesIn);

        _sendBackETHLeftover();

        emit SentUserBalance(_dstChainSelector, _localUser, remoteUser, uPoint, tsPoint);
    }
    ...
    function sendTotalSupply(uint64 _dstChainSelector, PayFeesIn _payFeesIn) external payable {
        ...
        _sendMesssageViaCCIP(uint64(_dstChainSelector), childChainReceiver, data, _payFeesIn);
    ...
    function _getBalanceAndTotalSupplyData(
        ...
        internal
        view
        returns (address remoteUser, bytes memory data, IVeSilo.Point memory tsPoint, IVeSilo.Point memory uPoint)
    {
        ...
        data = abi.encode(MessageType.BalanceAndTotalSupply, remoteUser, lockedEnd, uPoint, tsPoint);
    }
