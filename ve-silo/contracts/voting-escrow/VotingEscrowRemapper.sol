...
contract VotingEscrowRemapper is IVotingEscrowCCIPRemapper, Ownable2Step, ReentrancyGuard {
    ...
    function setNetworkRemapping(
        ...
        if (payFeesIn == ICCIPMessageSender.PayFeesIn.Native) {
            _transferAndPayFeesInNative(fee, oldRemoteUser, localUser, chainId, payFeesIn);
        } else {
            _transferAndPayFeesInLink(fee, oldRemoteUser, localUser, chainId, payFeesIn);
        }
    ...
    /// @inheritdoc IVotingEscrowCCIPRemapper
    function setNetworkRemappingManager(address localUser, address delegate) external override onlyOwner nonReentrant {
    ...
    function clearNetworkRemapping(
        ...
        if (payFeesIn == ICCIPMessageSender.PayFeesIn.Native) {
            _transferAndPayFeesInNative(fee, remoteUser, localUser, chainId, payFeesIn);
        } else {
            _transferAndPayFeesInLink(fee, remoteUser, localUser, chainId, payFeesIn);
        }
