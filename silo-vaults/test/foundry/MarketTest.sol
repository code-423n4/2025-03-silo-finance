...
contract MarketTest is IntegrationTest {
    ...
    */
    function testAllowanceOnSetCap() public {
        ...
        assertEq(asset.allowance(address(vault), address(market)), type(uint256).max, "allowance is in use");

        _setCap(market, 0);

        assertEq(asset.allowance(address(vault), address(market)), 0, "allowance is ZERO when cap is 0");
    }
    ...
    */
    function testSubmitCapInconsistentAsset() public {
        ...
        vm.mockCall(
            ...
            abi.encode(makeAddr("not loan token"))
        );
    ...
    function testUpdateWithdrawQueueInvalidMarketRemovalTimelockNotElapsed(uint256 elapsed) public {
        ...
        vm.prank(ALLOCATOR);
        vm.expectRevert(abi.encodeWithSelector(ErrorsLib.InvalidMarketRemovalTimelockNotElapsed.selector, idleMarket));
        vault.updateWithdrawQueue(indexes);
    }
