...
contract VeSiloDelegatorViaCCIPProposer is TwoStepOwnableProposer {
    ...
    constructor(address _proposal) Proposer(_proposal) {
        VESILO_DELEGATOR = VeSiloDeployments.get(VeSiloContracts.VE_SILO_DELEGATOR_VIA_CCIP, ChainsLib.chainAlias());

        if (VESILO_DELEGATOR == address(0))
            revert DeploymentNotFound(VeSiloContracts.VE_SILO_DELEGATOR_VIA_CCIP, ChainsLib.chainAlias());
    }
