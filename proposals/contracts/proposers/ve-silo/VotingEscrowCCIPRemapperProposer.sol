...
contract VotingEscrowCCIPRemapperProposer is TwoStepOwnableProposer {
    ...
    constructor(address _proposal) Proposer(_proposal) {
        VOTING_ESCROW_REMAPPER = VeSiloDeployments.get(VeSiloContracts.VOTING_ESCROW_REMAPPER, ChainsLib.chainAlias());

        if (VOTING_ESCROW_REMAPPER == address(0))
            revert DeploymentNotFound(VeSiloContracts.VOTING_ESCROW_REMAPPER, ChainsLib.chainAlias());
    }
