...
contract GaugeControllerProposer is Proposer {
    ...
    constructor(address _proposal) Proposer(_proposal) {
        GAUGE_CONTROLLER = VeSiloDeployments.get(VeSiloContracts.GAUGE_CONTROLLER, ChainsLib.chainAlias());

        if (GAUGE_CONTROLLER == address(0))
            revert DeploymentNotFound(VeSiloContracts.GAUGE_CONTROLLER, ChainsLib.chainAlias());
    }
