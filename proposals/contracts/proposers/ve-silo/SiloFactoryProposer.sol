...
contract SiloFactoryProposer is TwoStepOwnableProposer {
    ...
    constructor(address _proposal) Proposer(_proposal) {
        SILO_FACTORY = SiloCoreDeployments.get(SiloCoreContracts.SILO_FACTORY, ChainsLib.chainAlias());

        if (SILO_FACTORY == address(0))
            revert DeploymentNotFound(SiloCoreContracts.SILO_FACTORY, ChainsLib.chainAlias());
    }
