...
contract GaugeAdderProposer is Proposer {
    ...
    constructor(address _proposal) Proposer(_proposal) {
        GAUGE_ADDER = VeSiloDeployments.get(VeSiloContracts.GAUGE_ADDER, ChainsLib.chainAlias());

        if (GAUGE_ADDER == address(0)) revert DeploymentNotFound(VeSiloContracts.GAUGE_ADDER, ChainsLib.chainAlias());
    }
