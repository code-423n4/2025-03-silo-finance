...
contract FeeDistributorProposer is TwoStepOwnableProposer {
    ...
    constructor(address _proposal) Proposer(_proposal) {
        FEE_DISTRIBUTOR = VeSiloDeployments.get(VeSiloContracts.FEE_DISTRIBUTOR, ChainsLib.chainAlias());

        if (FEE_DISTRIBUTOR == address(0))
            revert DeploymentNotFound(VeSiloContracts.FEE_DISTRIBUTOR, ChainsLib.chainAlias());
    }
