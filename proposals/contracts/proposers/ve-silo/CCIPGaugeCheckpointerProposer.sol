...
contract CCIPGaugeCheckpointerProposer is TwoStepOwnableProposer {
    ...
    constructor(address _proposal) Proposer(_proposal) {
        CHECKPOINTER = VeSiloDeployments.get(VeSiloContracts.CCIP_GAUGE_CHECKPOINTER, ChainsLib.chainAlias());

        if (CHECKPOINTER == address(0))
            revert DeploymentNotFound(VeSiloContracts.CCIP_GAUGE_CHECKPOINTER, ChainsLib.chainAlias());
    }
