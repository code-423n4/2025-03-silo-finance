...
abstract contract VaultActions is IBaseRelayerLibrary {
     ...
     */
    function _doJoinPoolChainedReferenceReplacements(
        PoolKind kind,
    ...
    function _doComposableStableV2ExitChainedReferenceReplacements(
        bytes memory userData
    ) private returns (bytes memory) {
        uint8 exitKind = uint8(StablePoolUserData.exitKind(userData));
    ...
    function _doStableExactBptInForOneTokenOutReplacements(
        bytes memory userData,
        uint8 exitKind
    ) private returns (bytes memory) {
    ...
    function _doStableExactBptInForTokensOutReplacements(
        bytes memory userData,
        uint8 exitKind
    ) private returns (bytes memory) {
