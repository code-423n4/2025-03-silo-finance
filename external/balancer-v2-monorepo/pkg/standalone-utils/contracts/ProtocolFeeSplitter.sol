...
contract ProtocolFeeSplitter is IProtocolFeeSplitter, SingletonAuthentication {
    ...
    constructor(
        IProtocolFeesWithdrawer protocolFeesWithdrawer,
        address daoFundsRecipient
    ...
    function getRevenueShareSettings(
        bytes32 poolId
    ) external view override returns (uint256 revenueSharePercentageOverride, address beneficiary, bool overrideSet) {
    ...
    function _withdrawBpt(IERC20 bpt, uint256 amount, address to) private {
        if (amount == 0) {
            return;
        }
    ...
    function _computeAmounts(
        uint256 feeCollectorBptBalance,
        uint256 feePercentage
    ) private pure returns (uint256 ownerAmount, uint256 daoAmount) {
