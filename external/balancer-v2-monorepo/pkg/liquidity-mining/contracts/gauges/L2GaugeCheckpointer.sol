...
contract L2GaugeCheckpointer is IL2GaugeCheckpointer, ReentrancyGuard {
    ...
    ) external override withSupportedGaugeType(gaugeType) {
    ...
    ) external view override withSupportedGaugeType(gaugeType) returns (bool) {
    ...
    function getTotalGauges(
        IGaugeAdder.GaugeType gaugeType
    ) external view override withSupportedGaugeType(gaugeType) returns (uint256) {
    ...
    function getGaugeAtIndex(
        IGaugeAdder.GaugeType gaugeType,
        uint256 index
    ) external view override withSupportedGaugeType(gaugeType) returns (IStakelessGauge) {
    ...
    ) external payable override withSupportedGaugeType(gaugeType) nonReentrant {
        // solhint-disable-next-line not-rely-on-time
        uint256 currentPeriod = _roundDownTimestamp(block.timestamp);
