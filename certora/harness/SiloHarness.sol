...
contract SiloHarness is Silo {
    ...
    function getSiloDataInterestRateTimestamp() external view returns (uint64 interestRateTimestamp) {
    ...
    function getSiloDataDaoAndDeployerRevenue() external view returns (uint192 daoAndDeployerRevenue) {
    ...
    function getLTV(address borrower) external view returns (uint256) {
        (ISiloConfig.ConfigData memory collateralConfig, ISiloConfig.ConfigData memory debtConfig) = ShareTokenLib
            .siloConfig()
    ...
    function getAssetsDataForLtvCalculations(address borrower) external view returns (SiloSolvencyLib.LtvData memory) {
        ...
        return
            SiloSolvencyLib.getAssetsDataForLtvCalculations(
                collateralConfig,
                debtConfig,
                borrower,
                ISilo.OracleType.MaxLtv,
