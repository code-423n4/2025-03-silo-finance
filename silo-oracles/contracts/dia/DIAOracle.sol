...
contract DIAOracle is ISiloOracle, IDIAOracle, Initializable {
    ...
    function quote(uint256 _baseAmount, address _baseToken) external view virtual returns (uint256 quoteAmount) {
        ...
        (uint128 assetPrice, bool priceUpToDate) = getPriceForKey(
            data.diaOracle,
            primaryKey[cacheOracleConfig],
            data.heartbeat
        );
        ...
        if (!data.convertToQuote) {
            return
                OracleNormalization.normalizePrice(
                    _baseAmount,
                    assetPrice,
        ...
        (uint128 secondaryPrice, bool secondaryPriceValid) = getPriceForKey(
            data.diaOracle,
            secondaryKey[cacheOracleConfig],
            data.heartbeat
        );
        ...
        return
            OracleNormalization.normalizePrices(
                _baseAmount,
                assetPrice,
                secondaryPrice,
                data.normalizationDivider,
    ...
    function getPriceForKey(
        IDIAOracleV2 _diaOracle,
        string memory _key,
        uint256 _heartbeat
    ) public view virtual returns (uint128 assetPriceInUsd, bool priceUpToDate) {
        ...
        unchecked {
            oldestAcceptedPriceTimestamp = block.timestamp - _heartbeat;
        }
