...
contract PendlePTOracle is ISiloOracle {
    ...
    constructor(ISiloOracle _underlyingOracle, IPyYtLpOracleLike _pendleOracle, address _market) {
        ...
        (bool increaseCardinalityRequired, , bool oldestObservationSatisfied) = _pendleOracle.getOracleState(
            _market,
            TWAP_DURATION
        );
    ...
    function getPtToken(address _market) public view virtual returns (address ptToken) {
        (, ptToken, ) = IPendleMarketV3Like(_market).readTokens();
    }

    function getPtUnderlyingToken(address _market) public view virtual returns (address ptUnderlyingToken) {
