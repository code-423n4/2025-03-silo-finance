...
contract OracleForQA is ISiloOracle {
    ...
    function quoteToken() external view virtual override returns (address) {
        return QUOTE_TOKEN;
    }

    /// @inheritdoc ISiloOracle
    function quote(uint256 _baseAmount, address _baseToken) external view virtual returns (uint256 quoteAmount) {
        return _baseToken == QUOTE_TOKEN ? _baseAmount : (_baseAmount * priceOfOneBaseToken) / (10 ** BASE_DECIMALS);
    }
