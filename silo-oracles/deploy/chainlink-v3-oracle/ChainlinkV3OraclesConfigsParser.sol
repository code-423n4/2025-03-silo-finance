...
library ChainlinkV3OraclesConfigsParser {
    ...
    function getConfig(
        ...
        string memory _name
    ) internal returns (IChainlinkV3Oracle.ChainlinkV3DeploymentConfig memory config) {
        ...
        require(
            config.normalizationDivider != 0 || config.normalizationMultiplier != 0,
            "normalization variables not set"
        );
