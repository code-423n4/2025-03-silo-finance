...
contract Silo is ISilo, ShareCollateralToken {
    ...
    ) external payable virtual returns (bool success, bytes memory result) {
    ...
    /// @inheritdoc ISilo
    function getTotalAssetsStorage(AssetType _assetType) external view virtual returns (uint256 totalAssetsByType) {
    ...
    function deposit(
        ...
        (, shares) = _deposit({_assets: _assets, _shares: 0, _receiver: _receiver, _collateralType: _collateralType});
    }
    ...
    function mint(
        ...
        (assets, ) = _deposit({_assets: 0, _shares: _shares, _receiver: _receiver, _collateralType: _collateralType});
    }
    ...
    /// @inheritdoc ISilo
    function borrow(uint256 _assets, address _receiver, address _borrower) external virtual returns (uint256 shares) {
        uint256 assets;
        bool collateralTypeChanged;

        (assets, shares, collateralTypeChanged) = Actions.borrow(
            BorrowArgs({assets: _assets, shares: 0, receiver: _receiver, borrower: _borrower})
        );
    ...
    function borrowShares(
        ...
        (assets, shares, collateralTypeChanged) = Actions.borrow(
            BorrowArgs({assets: 0, shares: _shares, receiver: _receiver, borrower: _borrower})
        );
    ...
    function transitionCollateral(
        ...
        (assets, toShares) = Actions.transitionCollateral(
            TransitionCollateralArgs({shares: _shares, owner: _owner, transitionFrom: _transitionFrom})
        );
    ...
    /// @inheritdoc ISilo
    function repay(uint256 _assets, address _borrower) external virtual returns (uint256 shares) {
        uint256 assets;

        (assets, shares) = Actions.repay({_assets: _assets, _shares: 0, _borrower: _borrower, _repayer: msg.sender});
    ...
    /// @inheritdoc ISilo
    function repayShares(uint256 _shares, address _borrower) external virtual returns (uint256 assets) {
        uint256 shares;

        (assets, shares) = Actions.repay({_assets: 0, _shares: _shares, _borrower: _borrower, _repayer: msg.sender});
    ...
    function _previewMint(
        ...
        return
            SiloMathLib.convertToAssets(_shares, totalSiloAssets, totalShares, Rounding.DEPOSIT_TO_ASSETS, assetType);
    }
    ...
    function _previewDeposit(
        ...
        return
            SiloMathLib.convertToShares(_assets, totalSiloAssets, totalShares, Rounding.DEPOSIT_TO_SHARES, assetType);
    }
    ...
    function _previewRedeem(
        ...
        return
            SiloMathLib.convertToAssets(_shares, totalSiloAssets, totalShares, Rounding.WITHDRAW_TO_ASSETS, assetType);
    }
    ...
    function _previewWithdraw(
        ...
        return
            SiloMathLib.convertToShares(_assets, totalSiloAssets, totalShares, Rounding.WITHDRAW_TO_SHARES, assetType);
    }

    function _maxWithdraw(
        address _owner,
        ISilo.CollateralType _collateralType
    ) internal view virtual returns (uint256 assets, uint256 shares) {
