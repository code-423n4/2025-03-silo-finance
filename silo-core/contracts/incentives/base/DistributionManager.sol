...
contract DistributionManager is IDistributionManager, Ownable2Step {
    ...
    function incentivesProgram(
        string calldata _incentivesProgram
    ) external view virtual returns (IncentiveProgramDetails memory details) {
        bytes32 incentivesProgramId = getProgramId(_incentivesProgram);
     ...
     */
    function _accrueRewards(address _user) internal virtual returns (AccruedRewards[] memory accruedRewards) {
     ...
     * @return accruedRewards The accrued rewards for the user until the moment
     */
    function _accrueRewardsForPrograms(
        address _user,
    ...
    function _accrueRewards(
        ...
        uint256 rewards = _updateUserAssetInternal(_programId, _user, _userStaked, _totalStaked);
    ...
    function _getUnclaimedRewards(
        ...
        uint256 stakedByUser,
        uint256 totalStaked
    ) internal view virtual returns (uint256 accruedRewards) {
    ...
    function _getScaledUserBalanceAndSupply(
        address _user
    ) internal view virtual returns (uint256 userBalance, uint256 totalSupply) {
