...
abstract contract BaseIncentivesController is DistributionManager, ISiloIncentivesController {
    ...
    ) external view virtual returns (uint256 unclaimedRewards) {
    ...
    function _getRewardsBalance(
        address _user,
        bytes32 _programId,
        uint256 _stakedByUser,
        uint256 _totalStaked
    ) internal view virtual returns (uint256 unclaimedRewards) {
    ...
    function claimRewards(
        address _to,
        string[] calldata _programNames
    ) external virtual returns (AccruedRewards[] memory accruedRewards) {
        if (_to == address(0)) revert InvalidToAddress();
     ...
     */
    function _getProgramsIds(
        string[] calldata _programNames
    ) internal pure virtual returns (bytes32[] memory programIds) {
