...
library SiloSolvencyLib {
    ...
    function getAssetsDataForLtvCalculations(
        ...
        (uint256 totalCollateralAssets, uint256 totalProtectedAssets) = ISilo(_collateralConfig.silo)
    ...
    function calculateLtv(
        ...
        address _debtAsset
    ) internal view returns (uint256 sumOfBorrowerCollateralValue, uint256 totalBorrowerDebtValue, uint256 ltvInDp) {
        (sumOfBorrowerCollateralValue, totalBorrowerDebtValue) = getPositionValues(
            _ltvData,
            _collateralToken,
            _debtAsset
        );
    ...
    function getPositionValues(
        LtvData memory _ltvData,
        address _collateralAsset,
        address _debtAsset
    ) internal view returns (uint256 sumOfCollateralValue, uint256 debtValue) {
    ...
    ) internal pure returns (uint256 ltvInDp) {
