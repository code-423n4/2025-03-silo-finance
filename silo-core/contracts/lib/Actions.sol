...
library Actions {
    ...
    function deposit(
        ...
        (address shareToken, address asset) = siloConfig.getCollateralShareTokenAndAsset(
            address(this),
            _collateralType
        );
    ...
    function borrow(
    ...
    ) external returns (uint256 assets, uint256 shares, bool collateralTypeChanged) {
        ...
        (assets, shares) = SiloLendingLib.borrow(debtConfig.debtShareToken, debtConfig.token, msg.sender, _args);
    ...
    function transitionCollateral(
    ...
    ) external returns (uint256 assets, uint256 toShares) {
        ...
        (ISilo.CollateralType depositType, address shareTokenTo) = _args.transitionFrom ==
    ...
    function withdrawFees(
        ISilo _silo
    ) external returns (uint256 daoRevenue, uint256 deployerRevenue, bool redirectedDeployerFees) {
        ISiloConfig siloConfig = ShareTokenLib.siloConfig();
        siloConfig.turnOnReentrancyProtection();
        ...
        unchecked {
            $.daoAndDeployerRevenue -= uint192(earnedFees);
        }
    ...
    function _hookCallBeforeWithdraw(ISilo.WithdrawArgs calldata _args) private {
        ...
        bytes memory data = abi.encodePacked(_args.assets, _args.shares, _args.receiver, _args.owner, _args.spender);

        IHookReceiver(_shareStorage.hookSetup.hookReceiver).beforeAction(address(this), action, data);
    }

    function _hookCallAfterWithdraw(ISilo.WithdrawArgs calldata _args, uint256 assets, uint256 shares) private {
        IShareToken.ShareTokenStorage storage _shareStorage = ShareTokenLib.getShareTokenStorage();
