...
library SiloLensLib {
    ...
    function getLtv(ISilo _silo, address _borrower) internal view returns (uint256 ltv) {
        (ISiloConfig.ConfigData memory collateralConfig, ISiloConfig.ConfigData memory debtConfig) = _silo
            .config()
    ...
    function inDebt(ISiloConfig _siloConfig, address _borrower) internal view returns (bool has) {
        ...
        has =
            debtConfig.debtShareToken != address(0) &&
    ...
    function collateralBalanceOfUnderlying(
        ISilo _silo,
        address _borrower
    ) internal view returns (uint256 borrowerCollateral) {
    ...
    function calculateValues(
        ...
        (sumOfBorrowerCollateralValue, totalBorrowerDebtValue, ) = SiloSolvencyLib.calculateLtv(
            ltvData,
