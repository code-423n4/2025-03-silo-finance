...
library SiloMathLib {
    ...
    function getDebtAmountsWithInterest(
        ...
        uint256 _rcomp
    ) internal pure returns (uint256 debtAssetsWithInterest, uint256 accruedInterest) {
    ...
    function calculateMaxAssetsToWithdraw(
        ...
        unchecked {
            spareCollateralValue = _sumOfCollateralsValue - minimumCollateralValue;
        }

        maxAssets = (_borrowerProtectedAssets + _borrowerCollateralAssets).mulDiv(
            spareCollateralValue,
    ...
    function mulDivOverflow(uint256 _a, uint256 _b, uint256 _c) internal pure returns (uint256 mulDivResult) {
        if (_a == 0) return (0);
