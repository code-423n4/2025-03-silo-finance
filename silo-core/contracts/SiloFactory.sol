...
contract SiloFactory is ISiloFactory, ERC721, Ownable2Step {
    ...
    constructor(address _daoFeeReceiver) ERC721("Silo Finance Fee Receiver", "feeSILO") Ownable(msg.sender) {
        // start IDs from 1
        _siloId = 1;
    ...
    function createSilo(
        ...
        emit NewSilo(_siloImpl, silo0.asset(), silo1.asset(), address(silo0), address(silo1), address(_siloConfig));
    }
    ...
    function getFeeReceivers(
        address _silo
    ) external view virtual returns (address daoReceiver, address deployerReceiver) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return
            string.concat(baseURI, Strings.toString(block.chainid), "/", Strings.toHexString(idToSiloConfig[tokenId]));
    }
    ...
    function _validateShareTokens(
        ...
        (address protectedShareToken, address collateralShareToken, address debtShareToken) = _siloConfig
            .getShareTokens(_silo);
