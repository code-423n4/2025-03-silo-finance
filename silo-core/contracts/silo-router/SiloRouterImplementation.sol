...
contract SiloRouterImplementation is ISiloRouterImplementation {
    ...
    function sendValueAll(address payable _to) external payable virtual {
        ...
        if (balance != 0) {
            // expect this fn to be used as a sanity check at the end of the multicall
    ...
    /// @inheritdoc ISiloRouterImplementation
    function borrow(ISilo _silo, uint256 _assets, address _receiver) external payable virtual returns (uint256 shares) {
