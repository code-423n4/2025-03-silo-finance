...
library PartialLiquidationExecLib {
        ...
        )
    {
        ...
        (borrowerCollateralToLiquidate, repayDebtAssets, customError) = liquidationPreview(
            ltvData,
        ...
        (withdrawAssetsFromCollateral, withdrawAssetsFromProtected) = PartialLiquidationLib
            .splitReceiveCollateralToLiquidate(borrowerCollateralToLiquidate, ltvData.borrowerProtectedAssets);
    }
    ...
    function maxLiquidation(
        ...
        (ISiloConfig.ConfigData memory collateralConfig, ISiloConfig.ConfigData memory debtConfig) = _siloConfig
            .getConfigsForSolvency(_borrower);
        ...
        (uint256 sumOfCollateralValue, uint256 debtValue) = SiloSolvencyLib.getPositionValues(
            ltvData,
    ...
    function liquidationPreview(
    ...
    ) internal view returns (uint256 receiveCollateralAssets, uint256 repayDebtAssets, bytes4 customError) {
        ...
        (uint256 sumOfBorrowerCollateralValue, uint256 totalBorrowerDebtValue, uint256 ltvBefore) = SiloSolvencyLib
            .calculateLtv(_ltvData, _params.collateralConfigAsset, _params.debtConfigAsset);
