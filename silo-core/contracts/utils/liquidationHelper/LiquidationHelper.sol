...
contract LiquidationHelper is ILiquidationHelper, IERC3156FlashBorrower, DexSwap, TokenRescuer {
    ...
    constructor(address _nativeToken, address _exchangeProxy, address payable _tokensReceiver) DexSwap(_exchangeProxy) {
    ...
    function onFlashLoan(
        ...
        (LiquidationData memory _liquidation, DexSwapInput[] memory _swapInputs) = abi.decode(
            _data,
            (LiquidationData, DexSwapInput[])
        );

        IERC20(_debtAsset).forceApprove(address(_liquidation.hook), _maxDebtToCover);

        (_withdrawCollateral, _repayDebtAssets) = _liquidation.hook.liquidationCall({
