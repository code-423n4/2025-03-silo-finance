...
contract Logger is Test {
    ...
    function _logOracle(ISiloOracle _oracle, address _baseToken) internal {
        ...
        (address primaryAggregator, address secondaryAggregator) = _resolveUnderlyingChainlinkAggregators({
            _oracle: _oracle,
            _logDetails: false
        });
    ...
    function _logLatestRoundData(address _aggregator, bool _isPrimary) internal {
        ...
        (
            ,
            /*uint80 roundID*/ int256 aggregatorPrice,
            ,
            /*uint256 startedAt*/ uint256 priceTimestamp,
    ...
    function _resolveUnderlyingChainlinkAggregators(
        ISiloOracle _oracle,
        bool _logDetails
    ) internal view returns (address primaryAggregator, address secondaryAggregator) {
        try ChainlinkV3Oracle(address(_oracle)).oracleConfig() returns (ChainlinkV3OracleConfig oracleConfig) {
            (, bytes memory data) = address(oracleConfig).staticcall(
                abi.encodeWithSelector(ChainlinkV3OracleConfig.getConfig.selector)
            );
            ...
            if (data.length == _OLD_CHAINLINK_CONFIG_DATA_LEN) {
                ...
                IChainlinkV3Oracle.ChainlinkV3Config memory config = abi.decode(
                    data,
                    (IChainlinkV3Oracle.ChainlinkV3Config)
                );
    ...
    function _printPrice(
        ISiloOracle _oracle,
        address _baseToken,
        QuoteNamedAmount memory _quoteNamedAmount
    ) internal view {
    ...
    function _getAmountsToQuote(
        uint8 _baseTokenDecimals
    ) internal pure returns (QuoteNamedAmount[] memory amountsToQuote) {
        amountsToQuote = new QuoteNamedAmount[](10);
        uint256 oneToken = (10 ** uint256(_baseTokenDecimals));

        amountsToQuote[0] = QuoteNamedAmount({amount: 1, name: "1 wei (lowest amount)", logExponentialNotation: false});

        amountsToQuote[1] = QuoteNamedAmount({amount: 10, name: "10 wei", logExponentialNotation: false});

        amountsToQuote[2] = QuoteNamedAmount({amount: oneToken / 10, name: "0.1 token", logExponentialNotation: true});

        amountsToQuote[3] = QuoteNamedAmount({amount: oneToken / 2, name: "0.5 token", logExponentialNotation: true});
        ...
        amountsToQuote[8] = QuoteNamedAmount({amount: 10 ** 36, name: "10**36 wei", logExponentialNotation: true});
