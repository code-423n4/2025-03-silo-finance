...
contract PrintSiloAddresses is CommonDeploy {
    ...
    function printSiloConfigRelatedAddresses(ISiloConfig.ConfigData memory _siloConfig) internal view {
        ...
        address protectedShareTokensGauge = address(
            hookReceiver.configuredGauges(IShareToken(_siloConfig.protectedShareToken))
        );

        address collateralShareTokensGauge = address(
            hookReceiver.configuredGauges(IShareToken(_siloConfig.collateralShareToken))
        );

        address debtShareTokensGauge = address(hookReceiver.configuredGauges(IShareToken(_siloConfig.debtShareToken)));
