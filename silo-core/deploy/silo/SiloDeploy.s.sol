...
import {ChainlinkV3OraclesConfigsParser} from "silo-oracles/deploy/chainlink-v3-oracle/ChainlinkV3OraclesConfigsParser.sol";
...
contract SiloDeploy is CommonDeploy {
    ...
    function run() public virtual returns (ISiloConfig siloConfig) {
        ...
        siloConfig = siloDeployer.deploy(oracles, irmConfigData0, irmConfigData1, hookReceiver, siloInitData);
    ...
    function _getOracles(
        SiloConfigData.ConfigData memory _config,
        SiloConfigData _siloData
    ) internal returns (ISiloDeployer.Oracles memory oracles) {
    ...
    function _diaTxData(
        ...
        IDIAOracle.DIADeploymentConfig memory config = DIAOraclesConfigsParser.getConfig(chainAlias, _oracleConfigName);
    ...
    function _isChainlinkOracle(string memory _oracleConfigName) internal returns (bool isChainlinkOracle) {
        address baseToken = KV.getAddress(ChainlinkV3OraclesConfigsParser.configFile(), _oracleConfigName, "baseToken");
    ...
    ) internal virtual returns (ISiloDeployer.ClonableHookReceiver memory hookReceiver) {}

    function _printAndValidateDetails(ISiloConfig _siloConfig, ISiloConfig.InitData memory siloInitData) internal view {
        string memory chainAlias = ChainsLib.chainAlias();
