...
abstract contract BaseTest is BaseStorage, PropertiesConstants, StdAsserts, StdUtils {
    ...
    function _getUserAssets(address silo, address user) internal view returns (uint256) {
        (address protectedShareToken, ) = siloConfig.getCollateralShareTokenAndAsset(
            silo,
            ISilo.CollateralType.Protected
        );
        (address collateralShareToken, ) = siloConfig.getCollateralShareTokenAndAsset(
            silo,
        ...
        return
            ISilo(silo).convertToAssets(protectedShares, ISilo.AssetType.Protected) +
