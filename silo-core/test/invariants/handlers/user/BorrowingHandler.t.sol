    ...
    */
    ...
    function borrow(uint256 _assets, uint8 i, uint8 j) external setup {
        ...
        (success, returnData) = actor.proxy(
            target,
            abi.encodeWithSelector(ISilo.borrow.selector, _assets, receiver, address(actor))
        );
    ...
    function borrowSameAsset(uint256 _assets, uint8 i, uint8 j) external setup {
        ...
        (success, returnData) = actor.proxy(
            target,
            abi.encodeWithSelector(ISilo.borrowSameAsset.selector, _assets, receiver, address(actor))
        );
    ...
    function borrowShares(uint256 _shares, uint8 i, uint8 j) external setup {
        ...
        (success, returnData) = actor.proxy(
            target,
            abi.encodeWithSelector(ISilo.borrowShares.selector, _shares, receiver, address(actor))
        );
    ...
    function repayShares(uint256 _shares, uint8 i, uint8 j) external setup {
        ...
        (success, returnData) = actor.proxy(
            target,
            abi.encodeWithSelector(ISilo.repayShares.selector, _shares, borrower)
        );
    ...
    function transitionCollateral(uint256 _shares, uint8 i, uint8 j, uint8 k) external setup {
        ...
        uint256 _assets = ISilo(target).convertToAssets(
            _shares,
            (_collateralType == ISilo.CollateralType.Protected) ? ISilo.AssetType.Protected : ISilo.AssetType.Collateral
        );

        _before();
        (success, returnData) = actor.proxy(
            target,
            abi.encodeWithSelector(ISilo.transitionCollateral.selector, _shares, owner, _collateralType)
        );
    ...
    function assertBORROWING_HSPOST_F(uint8 i, uint8 j) external setup {
        ...
        (success, returnData) = actor.proxy(
            target,
            abi.encodeWithSelector(ISilo.borrow.selector, maxBorrow, receiver, address(actor))
        );
