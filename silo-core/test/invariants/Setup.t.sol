...
contract Setup is BaseTest, IERC721Receiver {
    ...
    function _deployProtocolCore() internal {
        ...
        (, address protectedCollateralToken0) = siloConfig.getCollateralShareTokenAndAsset(
            address(vault0),
            ISilo.CollateralType.Protected
        );

        (, address protectedCollateralToken1) = siloConfig.getCollateralShareTokenAndAsset(
            address(vault1),
            ISilo.CollateralType.Protected
        );
