...
contract SiloDeploy_Local is SiloDeployWithGaugeHookReceiver {
    ...
    function beforeCreateSilo(
        ...
        if (siloConfigOverride.solvencyOracle0 != address(0)) {
            console2.log(
                "[override] solvencyOracle0 %s -> %s",
                _config.solvencyOracle0,
                siloConfigOverride.solvencyOracle0
            );
        ...
        if (
            siloConfigOverride.hookReceiver != address(0) || siloConfigOverride.hookReceiverImplementation != address(0)
        ) {
...
contract SiloFixture is StdCheats, CommonBase {
    ...
    function deploy_local(
        ...
        external
        returns (ISiloConfig siloConfig, ISilo silo0, ISilo silo1, address token0, address token1, address hookReceiver)
    {
    ...
    function _deploy(
        SiloDeployWithGaugeHookReceiver _siloDeploy,
        string memory _configName
    )
        internal
        returns (ISiloConfig siloConfig, ISilo silo0, ISilo silo1, address token0, address token1, address hookReceiver)
