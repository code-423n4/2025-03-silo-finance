...
contract SiloIntegrationTest is VeSiloFeatures {
    ...
    function test_anvil_VeSiloWithSiloCoreAndSiloOracles() public {
        ...
        ISiloConfig siloConfig = ISiloConfig(
            SiloDeployments.get(getChainAlias(), SiloConfigsNames.SILO_FULL_CONFIG_TEST)
        );
    ...
    function _configureGaugeHookReceiver(address _hookReceiver, IShareToken _shareToken, address _gauge) internal {
        ...
        assertEq(
            address(GaugeHookReceiver(_hookReceiver).configuredGauges(_shareToken)),
            address(0),
            "Hook receiver should not be initialized"
        );
        ...
        assertEq(
            address(GaugeHookReceiver(_hookReceiver).configuredGauges(_shareToken)),
            _gauge,
            "Hook receiver should be initialized"
        );
    }
    ...
    function _depositIntoSilo(ISiloConfig _siloConfig, ISiloLiquidityGauge _gauge) internal {
        ...
        assertEq(_gauge.working_balances(_bob), amountToDeposit.decimalsOffsetPow(), "Should have working balance");
    }
    ...
    function _getHookReceiverForCollateralToken(
        ISiloConfig _siloConfig
    ) internal returns (address hookReceiver, address shareToken) {
        (, address silo1) = _siloConfig.getSilos();
