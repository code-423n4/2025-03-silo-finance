...
contract VeSiloFeatures is CommonSiloIntegration {
    ...
    // Ethereum
    bytes32 internal constant _BALANCER_POOL_ID = 0x9cc64ee4cb672bc04c54b00a37e1ed75b2cc19dd0002000000000000000004c1;
    ...
    function _executeProposal(address[] memory targets, uint256[] memory values, bytes[] memory calldatas) internal {
        ...
        uint256 proposalId = siloGovernor.propose(targets, values, calldatas, description);
        ...
        siloGovernor.queue(targets, values, calldatas, keccak256(bytes(description)));

        vm.warp(block.timestamp + 3_600);

        siloGovernor.execute(targets, values, calldatas, keccak256(bytes(description)));
    }
    ...
    function _activeteBlancerTokenAdmin() internal {
        stdstore.target(getAddress(SILO_TOKEN)).sig(Ownable.owner.selector).checked_write(address(balancerTokenAdmin));
