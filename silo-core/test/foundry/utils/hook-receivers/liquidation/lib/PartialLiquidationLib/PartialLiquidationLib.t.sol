...
contract PartialLiquidationLibTest is Test, MaxRepayRawMath {
    ...
    */
    function test_PartialLiquidationLib_calculateCollateralToLiquidate_pass() public {
        ...
        for (uint256 i; i < data.length; i++) {
            (uint256 collateralAssets, uint256 collateralValue) = PartialLiquidationLib.calculateCollateralsToLiquidate(
    ...
    */
    function test_PartialLiquidationLib_liquidationPreview_pass() public {
        ...
        for (uint256 i; i < data.length; i++) {
            ...
            assertEq(debtAssetsToRepay, data[i].output.debtAssetsToRepay, _concatMsg(i, "] expect debtAssetsToRepay"));
            assertEq(
                ltvAfterLiquidation,
    ...
    */
    function test_PartialLiquidationLib_maxLiquidationPreview_pass() public {
        ...
        for (uint256 i; i < data.length; i++) {
            ...
            assertEq(
                collateralAssetsToLiquidate / 3,
                collateralValueToLiquidate,
                _concatMsg(i, "collateral cross check")
            );
    ...
    */
    ...
    function _assetsChunk(
        uint256 _totalValue,
        uint256 _totalAssets,
        uint256 _chunkValue
    ) private pure returns (uint256 _chunkAssets) {
