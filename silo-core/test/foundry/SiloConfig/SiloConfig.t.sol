...
contract SiloConfigTest is Test {
    ...
    */
    function test_getShareTokens_fuzz(
        ...
        (address protectedShareToken, address collateralShareToken, address debtShareToken) = siloConfig.getShareTokens(
            _configData0.silo
        );
    ...
    */
    function test_getFeesWithAsset_fuzz(
        ...
        (uint256 daoFee, uint256 deployerFee, uint256 flashloanFee, address asset) = siloConfig.getFeesWithAsset(
            _configData0.silo
        );
    ...
    */
    function test_getConfigs_zero() public {
        ...
        (ISiloConfig.ConfigData memory collateralConfig, ISiloConfig.ConfigData memory debtConfig) = _siloConfig
            .getConfigsForSolvency(address(0));
    ...
    */
    ...
    function _mockShareTokensBlances(address _user, uint256 _balance0, uint256 _balance1) internal {
        vm.mockCall(_configDataDefault0.debtShareToken, abi.encodeCall(IERC20.balanceOf, _user), abi.encode(_balance0));

        vm.mockCall(_configDataDefault1.debtShareToken, abi.encodeCall(IERC20.balanceOf, _user), abi.encode(_balance1));
    }
}
