    ...
    */
    function test_debt_ratio() public {
        ...
        emit log_named_decimal_uint(
            "maxWithdraw(depositor) diff",
            silo1.maxWithdraw(makeAddr("depositor")) - withdrawBefore,
            18
        );
    }
    ...
    function _printBorrowRatio() internal {
        ...
        emit log_named_uint(
            "[silo1] share balanceOf(borrower) ",
            IShareToken(debtShare).balanceOf(makeAddr("borrower"))
        );
        emit log_named_uint("[silo1] silo1.maxRepay(borrower) ", silo1.maxRepay(makeAddr("borrower")));
        emit log_named_uint(
            "[silo1] share balanceOf(borrower2) ",
            IShareToken(debtShare).balanceOf(makeAddr("borrower2"))
        );
        emit log_named_uint("[silo1] silo1.maxRepay(borrower2) ", silo1.maxRepay(makeAddr("borrower2")));
        emit log_named_uint(
            "[silo1] share balanceOf(borrower3) ",
            IShareToken(debtShare).balanceOf(makeAddr("borrower3"))
        );
