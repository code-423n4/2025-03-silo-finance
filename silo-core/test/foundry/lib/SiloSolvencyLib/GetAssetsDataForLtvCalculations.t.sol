...
import {GetAssetsDataForLtvCalculationsTestData} from "silo-core/test/foundry/data-readers/GetAssetsDataForLtvCalculationsTestData.sol";
...
contract GetAssetsDataForLtvCalculationsTest is Test {
    ...
    function getData(
        GetAssetsDataForLtvCalculationsTestData.ScenarioData memory scenario
    )
        public
        returns (
        ...
        )
    {
        ...
        if (scenario.input.accrueInMemory) {
            interestRateModelMock.getCompoundInterestRateMock(
                silo0,
                block.timestamp,
                scenario.input.collateralConfig.compoundInterestRate
            );
        }
        ...
        if (scenario.input.accrueInMemory) {
            interestRateModelMock.getCompoundInterestRateMock(
                silo1,
                block.timestamp,
                scenario.input.debtConfig.compoundInterestRate
            );
        }
    }
