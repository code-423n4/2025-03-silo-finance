...
contract EchidnaSetup is SiloLittleHelper, Test {
    ...
    function _invariant_insolventHasDebt(
        address _user
    ) internal view returns (bool isSolvent, ISilo _siloWithDebt, ISilo _siloWithCollateral) {
        // _dumpState(_user);
    ...
    function _dumpState(address _actor) internal {
        ...
        emit log_named_decimal_uint(
            "convertToShares(maxBorrow0):",
            silo0.convertToShares(maxBorrow0, ISilo.AssetType.Debt),
            18
        );
        emit log_named_decimal_uint(
            "convertToShares(maxBorrow1):",
            silo1.convertToShares(maxBorrow1, ISilo.AssetType.Debt),
            18
        );
