...
abstract contract MaxLiquidationCommon is SiloLittleHelper, Test {
    ...
    function _assertLiquidationHookHasNoBalance() internal view {
        ...
        (address collateralShare, address protectedShare, address debtShare) = siloConfig.getShareTokens(
            address(silo0)
        );
        assertEq(
    ...
    function _executeLiquidationAndRunChecks(bool _sameToken, bool _receiveSToken) internal {
        ...
        if (_sameToken) {
            ...
            assertEq(siloBalanceBefore1 + repayDebtAssets, token1.balanceOf(address(silo1)), "debt was repay to silo");
        }
    }
