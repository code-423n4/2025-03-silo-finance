...
contract SiloHooksActionsTest is SiloLittleHelper, Test, HookMock {
    ...
    function _siloBorrowBothHooks(ISilo _silo, address _borrowerAddr, address _receiver, uint256 _amount) internal {
        vm.expectEmit(true, true, true, true);
    ...
    function _liquidationTest(bool _receiveSToken) internal {
        ...
        uint256 afterAction = beforeActions
        .addAction(Hook.shareTokenTransfer(Hook.PROTECTED_TOKEN)).addAction(Hook.shareTokenTransfer(Hook.DEBT_TOKEN)); // as we have protected deposit
