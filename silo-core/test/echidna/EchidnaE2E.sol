...
contract EchidnaE2E is Deployers, PropertiesAsserts {
    ...
    function maxDeposit_correctMax(uint8 _actorIndex) public {
        ...
        try actor.deposit(true, maxAssets) {} catch {
    ...
    function maxMint_correctMax(uint8 _actorIndex, bool _vaultZero) public {
        ...
        try actor.mint(_vaultZero, maxShares) {} catch {
    ...
    function maxWithdraw_correctMax(uint8 _actorIndex) public {
        ...
        if (maxAssets == 0) {
            (ISiloConfig.ConfigData memory collateralConfig, ISiloConfig.ConfigData memory debtConfig) = siloConfig
                .getConfigsForSolvency(address(actor));
    ...
    function maxRedeem_correctMax(uint8 _actorIndex) public {
        ...
        try actor.redeem(_vaultWithCollateral, maxShares) {} catch {
            assert(false);
        }
    }
    ...
    function maxBorrow_correctReturnValue(uint8 _actorIndex) public {
        ...
        try actor.borrow(true, maxAssets) {} catch {
            assert(false);
        }
    }
    ...
    function maxBorrowShares_correctReturnValue(uint8 _actorIndex) public {
        ...
        try actor.borrowShares(true, maxShares) {} catch {
            assert(false);
        }
    }
    ...
    function maxRepay_correctReturnValue(uint8 _actorIndex) public {
        ...
        try actor.repay(true, maxAssets) {} catch {
            assert(false);
        }
    }
    ...
    function maxRepayShares_correctReturnValue(uint8 _actorIndex) public {
        ...
        try actor.repayShares(true, maxShares) {} catch {
            assert(false);
        }
    }
    ...
    function maxLiquidation_correctReturnValue(uint8 _actorIndex) public {
        ...
        try secondActor.liquidationCall(address(actor), debtToRepay, false, siloConfig) {} catch {
            assert(false);
        }
    }
    ...
    function cannotPreventInsolventUserFromBeingLiquidated(uint8 _actorIndex, bool receiveShares) public {
        ...
        try liquidator.liquidationCall(address(actor), debtToRepay, receiveShares, siloConfig) {} catch {
    ...
    function _invariant_insolventHasDebt(address _user) internal returns (bool isSolvent, bool _vaultZeroWithDebt) {
    ...
    function _invariant_onlySolventUserCanRedeem(
        address _user
    ) internal returns (bool isSolvent, bool vaultZeroWithCollateral) {
        // _dumpState(_user);

        isSolvent = vault0.isSolvent(_user);

        (address protectedShareToken0, address collateralShareToken0, address debtShareToken0) = siloConfig
            .getShareTokens(address(_vault0));
    ...
    function _dumpState(address _actor) internal {
        ...
        (address protectedToken0, address collateralToken0, address debtShareToken0) = siloConfig.getShareTokens(
            _vault0
        );

        (address protectedToken1, address collateralToken1, address debtShareToken1) = siloConfig.getShareTokens(
            _vault1
        );
