...
contract Actor is PropertiesAsserts, IERC3156FlashBorrower {
    ...
    function liquidationCall(address borrower, uint256 debtToCover, bool receiveSToken, ISiloConfig config) public {
        (ISiloConfig.ConfigData memory collateralConfig, ISiloConfig.ConfigData memory debtConfig) = config
            .getConfigsForSolvency(borrower);
    ...
    function _prepareForDepositShares(
        bool _vaultZero,
        uint256 _shares,
        ISilo.CollateralType _assetType
    ) internal returns (Silo vault, uint256 amount) {
        vault = _vaultZero ? vault0 : vault1;
